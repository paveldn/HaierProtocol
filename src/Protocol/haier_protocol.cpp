        ...
        // Check incoming messages
        {
            ...
            if (messagesCount > 0)
            {
                ...
                if (handler != mMessageHandlersMap.end())
                    hres = handler->second(msgType, frame.frame.getData(), frame.frame.getDataSize());
                else
                    hres = mDefaultMessageHandler(msgType, frame.frame.getData(), frame.frame.getDataSize());
                mProcessingMessage = false;
    ...
    case ProtocolState::psWaitingWorAnswer:
        ...
        if ((std::chrono::steady_clock::now() > mAnswerTimeout))
        {
            ...
            if (handler != mTimeoutHandlersMap.end())
                hres = handler->second(mLastMessageType);
            else
        ...
        if (mTransport.available() > 0)
        {
            ...
            if (handler != mAnswerHandlersMap.end())
                hres = handler->second(mLastMessageType, msgType, frame.frame.getData(), frame.frame.getDataSize());
            else
